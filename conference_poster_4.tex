...
\vspace{-0.3cm}
    ...
    $$
    X_{new}X^\top X_{new} = X^TX + x_{t+1}x_{t+1}^\top
    $$
    $$
    X_{new}^\top Y_{new} = X^T Y + y_{t+1} x_{t+1}
    $$
...
\headerbox{GP \& Kalman Filter}{name=introduction,column=1,row=0,bottomaligned=objectives}{
...
\headerbox{GP \& Kalman Filter}{name=introduction,column=2,row=0,bottomaligned=objectives}{
...
\begin{itemize}
    ...
    $=  \int_{\mathbb  R^d} p(\omega)e^{j\omega^T(x-x')}dw$ 
...
\colorbox[HTML]{CCFFFF}{\makebox[\textwidth-2\fboxsep][l]{\bf - Kalman Filter}}
$$
...
\vspace{-0.3cm}
\begin{itemize}
    \item maximizing information gain in homoscedastic noise case
    \item $\underset{t\rightarrow \infty}{lim}~\hat x_t = \underset{\in D}{argmax}~\mu_t(x)$, $f(\hat x )\rightarrow f(x^*)$
\end{itemize}
...
$$
\vspace{-0.3cm}
\includegraphics[width=0.8\textwidth, trim={0cm 1cm 0 0.2cm},clip]{figures/yzAfp5mekGTuO7w.png}
...
\begin{itemize}\compresslist
    \item guarantee converge to an $\varepsilon$ optimal policy not the exact optimal policy
    \item polynomial time, performance depend on the input
\end{itemize}
\underline{\textbf{Policy Iteration}}
\begin{itemize}\compresslist
    \item guaranteed to  monotonically improve the policy
\end{itemize}
