...
\vspace{-0.3cm}
    ...
    $$
    X_{new}X^\top X_{new} = X^\top Y + x_{t+1}x_{t+1}^\top
    $$
    $$
...
\vspace{-0.3cm}
    ...
    $$
\begin{itemize}\compresslist
    \item posterior is not gaussian not closed
    \item posterior cannot cover by variational infer
\end{itemize}
}
...
\begin{itemize}
    ...
    $=  \int_{\mathbb  R^d} p(\omega)e^{j\omega^top(x-x')}dw$ 
    ...
    \item inducing points: $\mathcal O(n^3)$ 
        ...
        FITC: cubic inducing points, liner point
\end{itemize}
...
\textbf{Wiener process/Brownian motion}
